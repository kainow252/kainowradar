...
function _shCleanup() {
  if (_shPollTimer) { clearInterval(_shPollTimer); _shPollTimer = null }
  if (_shLoginWin && !_shLoginWin.closed) { try { _shLoginWin.close() } catch(e){} }
  _shLoginWin = null
}
        ...
        <div id="si-panel-paste">
          ...
          <div id="si-mode-single-area">
            ...
            <textarea id="si-textarea" rows="6"
              ...
              oninput="siCountLinks()" onpaste="setTimeout(siCountLinks,50)" ondrop="setTimeout(siCountLinks,50)"></textarea>
...
async function siImportAuto(storeId) {
  ...
  try {
    ...
    const metaMap = {}
    // Para volumes grandes (>15 links), aumenta BATCH para terminar mais rápido
    const BATCH = urls.length > 30 ? 6 : urls.length > 15 ? 5 : 4
    let done = 0
    ...
    for (let s = 0; s < urls.length; s += BATCH) {
      ...
      batch.forEach((u, bi) => {
        ...
        if (tEl) tEl.textContent = pct < 100 ? 'Buscando ' + done + '/' + urls.length + '...' : '\u2713 Pronto!'
      })
    }
...
async function siFetchMetaClientSide(originalUrl, affiliateUrl) {
  ...
  // ── CAMADA 4: fallback allorigins.win (links não-ML, não-Amazon) ─
  if (!isML && !isAmazonUrl) {
